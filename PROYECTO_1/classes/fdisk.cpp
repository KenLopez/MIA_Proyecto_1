...
void Fdisk::execute(vector<PARAMETER*>* params){
    ...
    for (i = 0; i < params->size(); i++)
    {
        ...
        case pPATH:
            if(path!=""){
                cout<<"\033[91mERROR FDISK: Parámetro path ya fue definido.\033[0m"<<endl;
                return;
            }else if(p->text==""){
                cout<<"\033[91mERROR FDISK: Ruta no válida.\033[0m"<<endl;
                return;
            }
            path = p->text;
            break;
        ...
        case pSIZE:
            if(size!=-1){
                cout<<"\033[91mERROR FDISK: Parámetro size ya fue definido.\033[0m"<<endl;
                return;
            }else if(p->value == 0){
                cout<<"\033[91mERROR FDISK: Size debe ser mayor a 0.\033[0m"<<endl;
                return;
            }
            size = p->value;
            break;
        case pNAME:
            if(name!=""){
                cout<<"\033[91mERROR FDISK: Parámetro name ya fue definido.\033[0m"<<endl;
                return;
            }else if(p->text==""){
                cout<<"\033[91mERROR FDISK: Nombre no válido.\033[0m"<<endl;
                return;
            }else if(strlen(p->text)>16){
                cout<<"\033[91mERROR FDISK: Nombre excede la cantidad máxima de caracteres [16].\033[0m"<<endl;
                return;
            }
            name = p->text;
            break;
        ...
        case pTYPE:
            if(type!=tNONE){
                cout<<"\033[91mERROR FDISK: Parámetro type ya fue definido.\033[0m"<<endl;
                return;
            }
            type = p->param;
            break;
        ...
        case pUNIT:
            if(unit!=tNONE){
                cout<<"\033[91mERROR FDISK: Parámetro unit ya fue definido.\033[0m"<<endl;
                return;
            }
            unit = p->param;
            break;
        ...
        case pDELETE:
            if(delet!=tNONE){
                cout<<"\033[91mERROR FDISK: Parámetro delete ya fue definido.\033[0m"<<endl;
                return;
            }
            delet = p->param;
            break;
        ...
        case pFIT:
            if(fit!=tNONE){
                cout<<"\033[91mERROR FDISK: Parámetro fit ya fue definido.\033[0m"<<endl;
                return;
            }
            fit = p->param;
            break;
        ...
        case pADD:
            if(add != 0){
                cout<<"\033[91mERROR FDISK: Parámetro add ya fue definido.\033[0m"<<endl;
                return;
            }else if(p->param == 0){
                cout<<"\033[91mERROR FDISK: Parámetro add debe ser diferente de 0.\033[0m"<<endl;
                return;
            }
            add = p->value;
            break;
        default:
            cout<<"\033[91mERROR FDISK: Comando fdisk posee parámetros incorrectos.\033[0m"<<endl;
            return;
        }
    }
    if((size==-1 && add==0 && delet==tNONE) || path=="" || name==""){
        cout<<"\033[91mERROR FDISK: Comando fdisk no posee parámetros obligatorios (size / add / delete; path, name).\033[0m"<<endl;
        return;
    }
    if(name.length()>16){ cout<<"\033[91mERROR FDISK: Nombre de partición supera la longitud máxima (16).\033[0m"<<endl; return; }
    ...
    if(file = fopen(path.c_str(), "r+")){
        ...
        if(size>0){
            if(!checkPartName()){
                cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser creada, existe una partición "; 
                cout << "con el mismo nombre.\033[0m"<<endl;
                return;
            }
            switch (type)
            {
            case tEXTENDED:
                if(partition){
                    cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser creada, partición "; 
                    cout << "extendida ya fue creada ("<<partition->part_name<<").\033[0m"<<endl;
                    break;
                }
                ...
                if(checkPrimary()){
                    ...
                    if(free.size()>0){
                        if(allocatePartition(free)){
                            ...
                            cout << "\033[92mLa partición "<<name<<" ha sido creada exitosamente en "<<path<<".\033[0m"<<endl; 
                            break;
                        }
                    }
                    cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser creada en "<<path<<", espacio insuficiente para alojar.\033[0m"<<endl; 
                    break;
                }else{
                    cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser creada en "<<path<<", la capacidad máxima "; 
                    cout << "de particiones primarias alcanzó su límite.\033[0m"<<endl;
                    break;
                }
                break;
            ...
            case tLOGIC:
                if(partition){
                    ...
                    cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser creada en "<<path<<", partición "; 
                    cout << "extendida no fue encontrada.\033[0m"<<endl;
                }
                break;
            default:
                break;
        ...
        cout << "\033[91mERROR FDISK: El disco "<<getName(path)<<" no fue encontrado en la ruta "<<path<<".\033[0m"<<endl;
    }
}
...
void Fdisk::changeSize(){
    ...
    cout << "\033[91mERROR FDISK: La partición "<<name<<" no fue encontrada en la ruta "<<path<<".\033[0m"<<endl;
}
    ...
    if(add>0){
        ...
        if(next->part_start!=primaries[index]->part_start){
            if(next->part_start-(primaries[index]->part_start+primaries[index]->part_size)>=add){
                ...
                newPrimary();
                cout << "\033[92mLa partición "<<name<<" ha sido ampliada exitosamente en "<<path<<".\033[0m"<<endl; 
            }else{
                cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser ampliada en "<<path<<", no existe suficiente espacio disponible.\033[0m"<<endl;
            }
        ...
        if(result>0){
            ...
            newPrimary();
            cout << "\033[92mLa partición "<<name<<" ha sido encogida exitosamente en "<<path<<".\033[0m"<<endl; 
        }else{
            cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser encogida en "<<path<<", espacio a encoger supera tamaño de partición.\033[0m"<<endl;
        }
    }   
}
...
void Fdisk::changeLogic(int index){
    if(add>0){
        ...
        if(result>0 && result>sizeof(EBR_STRUCT)){
            ...
            cout << "\033[91mERROR FDISK: La partición "<<name<<" no pudo ser encogida en "<<path<<", espacio a encoger supera tamaño mínimo requerido (";
            cout<<sizeof(EBR_STRUCT)<<")\033[0m"<<endl;
        }
    }
}
...
void Fdisk::deletePrimary(int index){
    if(primaries[index]->part_type == 'E'){
        if(logics.size()>0){
            cout<<"\033[91mERROR FDISK: La particion "<<primaries[index]->part_name<<" no pudo ser eliminada, pues es una partición extendida y existen particiones lógicas creadas.\033[0m"<<endl;
            return;
        }
    }
    switch (delet)
...
void Fdisk::deletePartition(){
    int i = searchPrimary();
     if(i!=-1){
        deletePrimary(i);
        cout << "\033[92mLa partición "<<name<<" ha sido borrada exitosamente en "<<path<<".\033[0m"<<endl; 
        return;
    }
    i = searchLogic();
    if(i!=-1){
